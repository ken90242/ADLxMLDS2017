...
# 根據不同的model(W1, W2)建出相應的zero martrix
...
def policy_forward(x):
  ...
  logp = np.dot(model['W2'], h) # dot不只可以用在矩陣相乘，向量相乘也可以用
  p = sigmoid(logp)
...
while True:
  ...
  # 這邊輸入的圖像並不是實際的畫素，而是這個frame跟上個frame相減的畫素
  ...
  if done: # an episode finished
    ...
    epx = np.vstack(xs)
    # print(np.array(xs).shape, epx.shape)
    eph = np.vstack(hs)
    # print(np.array(hs).shape, eph.shape)
    epdlogp = np.vstack(dlogps)
    # print(np.array(dlogps).shape, epdlogp.shape)
    epr = np.vstack(drs)
    # print(np.array(drs).shape, epr.shape)
    ...
    # boring learning curve-storing
    if episode_number % 30 == 0:
      with open ('learning_curve.txt', 'a') as f:
        f.write(str(episode_number) + ', ' + str(running_reward) + '\n')

  if reward != 0: # Pong has either +1 or -1 reward exactly when game ends.
    print (str('ep %d: game finished, reward: %f' % (episode_number, reward)) + ('' if reward == -1 else ' !!!!!!!!'))
